...
import { enhanceHtmlDeliverable } from "@/lib/html-enhancer";
import {
...
export async function POST(request: Request) {
  ...
  try {
    await logger.info("html_request_received", {
    ...
    if (
      ...
      return Response.json(
        {
          error: "Upload a self-contained HTML file before enhancing.",
          logId: logger.id,
          logPath: logger.filePath,
        },
        { status: 400 },
    ...
    const job = {
      ...
      fileName,
      html,
      instructions: stringValue(formData.get("instructions")),
      model: selectedModel,
      ownerId: user?.id,
      provider: selectedProvider,
    ...
    after(async () => {
      try {
        const result = await enhanceHtmlDeliverable({
          ...job,
          logger,
        });

        await logger.info("html_background_job_completed", {
    ...
    await logger.info("html_background_job_started", {
      model: selectedModel,
      provider: selectedProvider,
      runId,
    });
    ...
    await logger.error("html_request_failed", serializeError(error));
    ...
    return Response.json(
      {
        error: "The HTML enhancer failed unexpectedly.",
        logId: logger.id,
        logPath: logger.filePath,
      },
      { status: 500 },
