      ...
      {solutions.map((group, gi) => (
        <div key={gi}>
          ...
          {group.steps.map((step, si) => (
            <div
              className="flex gap-[18px] items-start border-b border-slate-100 py-[18px] last:border-b-0"
              key={si}
            >
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#2f65a7] text-base font-black text-white shadow-[0_8px_18px_rgba(47,101,167,0.22)]">
                {si + 1}
              </div>
