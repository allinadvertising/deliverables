...
- **No ClickUp links, scoped work.** Omit `taskUrl` from `completedWork` so no ClickUp link renders in the report. Draw completed and in-progress work only from the subtasks of the month's task bundle (the parent of the SEO Monthly Report task, e.g. "August 2026 SEO Tasks"); do not use tasks from elsewhere in the list.
